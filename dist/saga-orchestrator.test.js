...
const saga_orchestrator_1 = require("./saga-orchestrator");
const in_memory_saga_store_1 = require("./in-memory-saga-store");
function makeStep(stepId, opts) {
    return {
        stepId,
...
describe('SagaOrchestrator', () => {
    ...
    it('happy path: runs all steps in order and marks the saga COMPLETED', async () => {
        const store = new in_memory_saga_store_1.InMemorySagaStore();
        ...
        const orchestrator = new saga_orchestrator_1.SagaOrchestrator(steps, store);
    ...
    it('a step failure triggers compensation of completed steps in reverse order, and steps with no compensate() are skipped', async () => {
        ...
        expect(callOrder.indexOf('DebitAccount.compensate')).toBeGreaterThan(callOrder.indexOf('SubmitToNetwork.execute'));
        const finalState = await store.load(SAGA_ID);
    ...
    it('dual-write scenario: a dangling PENDING checkpoint causes an idempotent re-invocation, not a silent skip and not a duplicate side effect', async () => {
        ...
        const debitExecute = jest.fn().mockImplementation(async (context) => {
    ...
    it('writes a PENDING checkpoint before calling execute(), not only after', async () => {
        ...
        expect(writeCheckpointSpy).toHaveBeenCalledWith(SAGA_ID, expect.objectContaining({ stepId: 'BalanceCheck', status: 'PENDING' }));
        expect(writeCheckpointSpy).toHaveBeenCalledWith(SAGA_ID, expect.objectContaining({ stepId: 'BalanceCheck', status: 'COMPLETED' }));
    });
});
